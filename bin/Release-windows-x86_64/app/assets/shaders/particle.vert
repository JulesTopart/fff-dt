...
void main() {
	...
	if(colorCycle == 0){ 
		...
		vec4 colorA = vec4(0.8, 0.0, 0.0, 1);
		vec4 colorB = vec4(0.0, 0.0, 0.8, 1);

		int curr_phaseAB = phaseAB(i);

		if(curr_phaseAB == 0){
			color = colorA;
		}else if(curr_phaseAB == 1){
			color = colorB;
		}
		...
		//color = vec4(randomColor(binindex), 1);
		mat3 stress =  mat3(b_sigma_i[0].xyz, b_sigma_i[1].xyz, b_sigma_i[2].xyz);
		//mat3 pl =  mat3(b_Pli[0].xyz, b_Pli[1].xyz, b_Pli[2].xyz);
		color = colorMap(map(computeVonMisesStress(stress),0,2.5*1e3), jet);
	}else{ //NNS Test
	...
	if( test || !binTest){
		...
		gl_PointSize = 1.8*radius*cst_particleRadius*600.0/(gl_Position.w);
